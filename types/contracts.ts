...
export function convertFormToContractData(formData: PoolFormData): PoolCreationData {
  ...
  if (!formData.odds || isNaN(oddsValue) || oddsValue <= 0) {
    throw new Error('Odds must be a valid positive number');
  }

  return {
    predictedOutcome: formData.predictedOutcome,
    odds: BigInt(Math.floor(oddsValue * 100)), // Convert to basis points
