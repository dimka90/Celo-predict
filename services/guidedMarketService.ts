...
export class GuidedMarketService {
  ...
  static async getCryptocurrencies(limit: number = 500): Promise<Cryptocurrency[]> {
    try {
      ...
      console.log('🎯 Fetching cryptos from:', apiUrl);
      ...
      const validCoins = coins
        .filter((crypto: any) => {
          // Accept either price_usd or price field
          const price = crypto.price_usd || crypto.price;
          return price && price > 0;
        })
        .sort((a: any, b: any) => (a.rank || 999) - (b.rank || 999))
        .slice(0, limit);
      
      console.log('✅ Valid coins:', validCoins.length);
      
      return validCoins.map((crypto: any) => ({
        id: crypto.id || crypto.symbol?.toLowerCase() || '',
        symbol: crypto.symbol || '',
        name: crypto.name || '',
        currentPrice: crypto.price_usd || crypto.price || 0,
        price_usd: crypto.price_usd || crypto.price,
        logo: crypto.logo_url || crypto.logo || this.getCryptoLogoUrl(crypto.id || crypto.symbol),
  ...
  static async getPopularCryptocurrencies(): Promise<Cryptocurrency[]> {
    try {
      ...
      console.log('🎯 Fetching popular cryptos from:', apiUrl);
      const response = await fetch(apiUrl);
      const data = await response.json();
      
      console.log('📦 Popular crypto response:', data);
