...
import CryptoMarketModal, { CryptoMarketData } from './CryptoMarketModal';
...
export default function CreateCryptoMarketForm({ onSuccess, onClose }: CreateCryptoMarketFormProps) {
  ...
  const [selectedAssetForModal, setSelectedAssetForModal] = useState<{ symbol: string; name: string; icon: string; currentPrice?: number } | null>(null);
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      const poolData = {
        ...
        description: `${formData.category} - ${formData.league}`,
      ...
      // Create pool (SimplePoolCreator doesn't support boost yet)
  ...
  }, [isConnected, address, validateForm, createPool, formData, generatePredictedOutcome, onSuccess, onClose]);
  ...
  return (
    <div className="bg-gray-900 rounded-lg p-6 max-w-2xl mx-auto">
      ...
      <CryptoMarketModal
        ...
        onSubmit={(data: CryptoMarketData) => {
          ...
          // Update event times based on timeframe
          const times = calculateEventTimes(data.timeFrame);
          setFormData(prev => ({
            ...prev,
            eventStartTime: times.eventStartTime,
            eventEndTime: times.eventEndTime,
