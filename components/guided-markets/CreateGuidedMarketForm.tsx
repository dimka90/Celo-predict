...
export default function CreateGuidedMarketForm({ onSuccess, onClose }: CreateGuidedMarketFormProps) {
  ...
  const [formData, setFormData] = useState<PoolFormData>({
    predictedOutcome: '',
    odds: '2.0', // Default odds: 2.0 (even money)
    creatorStake: '100',
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    if (isNaN(stakeAmount) || stakeAmount <= 0) {
      toast.error('Invalid creator stake amount');
      return;
    }
    ...
    try {
      // Create pool with SimplePoolCreator (simplified format)
      const txHash = await createPool({
        title: formData.title || `${formData.homeTeam} vs ${formData.awayTeam}`,
        description: `${formData.league} - ${formData.marketType}`,
        creatorStake: BigInt(Math.floor(stakeAmount * 1e18)),
        eventStartTime: BigInt(Math.floor(startTime.getTime() / 1000)),
        eventEndTime: BigInt(Math.floor(endTime.getTime() / 1000)),
      });
  ...
  }, [isConnected, address, validateForm, createPool, formData, onSuccess, onClose]); // ✅ FIX: Removed createPoolWithBoost from deps
  ...
  return (
    <div className="bg-gray-900 rounded-lg p-6 max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-2">
            Creator Stake ({formData.usePrix ? 'PRIX' : 'CELO'})
          </label>
          <input
            type="number"
            step="0.01"
          ...
          />
          <p className="text-xs text-gray-400 mt-1">
            Minimum: {formData.usePrix ? "1000 PRIX" : "5 CELO"}
          </p>
        </div>
