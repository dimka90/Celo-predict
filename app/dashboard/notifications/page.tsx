...
import { formatCELO, formatRelativeTime } from "@/utils/formatters";
import {
  BellIcon,
  TrophyIcon,
  BanknotesIcon,
...
export default function Page() {
  ...
  const activities = positions.map(position => {
    ...
    if (position.status === 'won') {
      ...
      color = 'green';
      message = `Won ${formatCELO(position.payoutAmount || position.prizeAmount || '0')} from "${position.title}"`;
      ...
      color = 'cyan';
      message = `Placed ${formatCELO(position.amount)} on "${position.title}"`;
    }
