...
import { useSimplePoolCreator } from "@/hooks/useSimplePoolCreator";
import { 
  ArrowLeftIcon, 
  XMarkIcon,
  ChartBarIcon,
...
function CreateMarketPageContent() {
  ...
  const { createPool } = useSimplePoolCreator();
  ...
  // Helper to get currency name (CELO based on network)
  const getCurrencyName = useCallback(() => {
    return 'CELO'; // Celo uses CELO on both mainnet and testnet
  }, []);

  const [data, setData] = useState<GuidedMarketData>({
    category: '',
    odds: 200, // 2.0x default
    creatorStake: 1, // Default to minimum CELO stake (1 CELO)
    description: ''
  });
  ...
  // Set default event start time when cryptocurrency is selected
  useEffect(() => {
    if (data.category === 'cryptocurrency' && !data.userEventStartTime) {
      const defaultTime = new Date(Date.now() + 5 * 60000); // 5 minutes from now
      setData(prev => ({
        ...prev,
        userEventStartTime: defaultTime
      }));
    }
  }, [data.category]);
  ...
  useEffect(() => {
    ...
    if (isSuccess && currentTxHash && successProcessedRef.current !== currentTxHash) {
      ...
      const creationFee = currentUsePrix ? '50 PRIX' : '0.01 CELO';
      const boostCost = currentBoostTier && currentBoostTier !== 'NONE' 
        ? `${currentBoostTier === 'BRONZE' ? '2' : currentBoostTier === 'SILVER' ? '3' : '5'} CELO`
        : '0';
  ...
  const validateStep = (stepNumber: number): boolean => {
    ...
    if (stepNumber === 2) {
      ...
      const minStakeCELO = 1; // 1 CELO minimum from contract
      const minStakePRIX = 1000; // 1000 PRIX minimum from contract
      const minStake = usePrix ? minStakePRIX : minStakeCELO;
  ...
  const handleCreateMarket = async () => {
    ...
    try {
      ...
      if (data.category === 'football' && data.selectedFixture) {
        ...
        // Always use the later of matchDate or minStartTime to ensure it's in the future
        ...
        const minStakeCELO = parseUnits("1", 18); // ✅ FIX: 1 CELO minimum (matches contract)
        ...
        if (!usePrix && creatorStake < minStakeCELO) {
          throw new Error(`Minimum stake for CELO pools is 1 CELO. You provided ${data.creatorStake} CELO.`);
        }
        ...
        const poolData = {
          ...
          description: `${data.selectedFixture.league.name} - ${data.outcome || 'Match Prediction'}`,
        ...
        console.log('🔍 Stake validation:', {
          ...
          minRequired: usePrix ? '1000000000000000000000' : '1000000000000000000', // ✅ FIX: 1 CELO minimum
          meetsMinimum: usePrix ? poolData.creatorStake >= BigInt('1000000000000000000000') : poolData.creatorStake >= BigInt('1000000000000000000') // ✅ FIX: 1 CELO minimum
        });
        ...
        const poolData = {
          ...
          description: `${data.direction} ${data.targetPrice} USD in ${data.timeframe}`,
          creatorStake: creatorStake,
          eventStartTime: BigInt(Math.floor(eventStartTime.getTime() / 1000)),
          eventEndTime: BigInt(Math.floor(eventEndTime.getTime() / 1000)),
        };
        ...
        <div>
          ...
          {data.selectedCrypto && (
            <div className="mt-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-3">Event Start Time*</label>
                <div className="relative">
                  <input
                    type="datetime-local"
                    value={data.userEventStartTime ? (() => {
                      const d = new Date(data.userEventStartTime);
                      ...
                      // Default to 5 minutes from now if not set
                    ...
                    className="w-full px-4 py-3 bg-gray-800/80 border-2 border-gray-600 rounded-lg text-white text-base placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all cursor-pointer hover:border-gray-500"
                    required
                  />
                </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <motion.button
                ...
                onClick={() => {
                  setUsePrix(false);
                  // Update stake to minimum CELO if current stake is below CELO minimum
              ...
              >
                <div className="font-semibold text-sm sm:text-base">CELO</div>
                ...
                <div className="text-right">
                  <div className="text-lg font-bold text-white">
                    ...
                    <span className="text-sm text-gray-400 ml-1">
                      CELO {/* ✅ FIX: Boost is always paid in CELO, not PRIX */}
                    </span>
                  </div>
                  <div className="text-xs text-gray-400">
                    + Creation Fee
          ...
          <div className="mb-4 p-3 bg-gray-700/50 rounded-lg border border-gray-600">
            ...
            <div className="space-y-1 text-xs sm:text-sm">
              <div className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-2">
                ...
                <span className="text-white font-medium">{usePrix ? '50 PRIX' : '0.01 CELO'}</span>
              </div>
              {data.boostTier && data.boostTier !== 'NONE' && (
                <div className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-2">
                  <span className="text-gray-300">Boost Fee ({data.boostTier}):</span>
                  <span className="text-white font-medium">
                    {data.boostTier === 'BRONZE' ? '2' : data.boostTier === 'SILVER' ? '3' : '5'} CELO
                  </span>
                </div>
              )}
              <div className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-2 border-t border-gray-600 pt-1">
                ...
                <span className="text-white font-bold">
                  {data.boostTier && data.boostTier !== 'NONE' 
                    ? `${(() => {
                        ...
                        return `${boostAmount} CELO + ${creationAmount} ${usePrix ? 'PRIX' : 'CELO'}`;
                      })()}`
                    : `${usePrix ? '50 PRIX' : '0.01 CELO'}`}
                </span>
              </div>
              <div className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-2 mt-2 pt-2 border-t border-gray-600">
      ...
      return (
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-center gap-2 sm:gap-4 flex-wrap sm:flex-nowrap">
            {[1, 2, 3].map((stepNumber) => (
              <React.Fragment key={stepNumber}>
                ...
                {stepNumber < 3 && (
                  <div className={`h-1 w-8 sm:w-16 flex-shrink-0 ${
          ...
          <motion.div
            ...
            className="glass-card p-4 sm:p-8 border border-border-card"
          >
